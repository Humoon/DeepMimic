...
class AMPAgent(PPOAgent):
    ...
    def _build_nets(self, json_data):
        ...
        disc_init_output_scale = 1 if (self.DISC_INIT_OUTPUT_SCALE_KEY
        ...
        with tf.variable_scope(self.MAIN_SCOPE):
            with tf.variable_scope(self.DISC_SCOPE):
                ...
                self._disc_logits_agent_tf = self._build_disc_net(
                    disc_net_name, self._disc_agent_inputs, disc_init_output_scale, reuse=True)
    ...
    def _build_losses(self, json_data):
        ...
        disc_logit_reg_weight = 0 if (self.DISC_LOGIT_REG_WEIGHT_KEY
        ...
        if (disc_grad_penalty != 0):
            self._grad_penalty_loss_tf = self._disc_grad_penalty_loss(
                in_tfs=self._disc_expert_inputs, out_tf=self._disc_logits_expert_tf)
    ...
    def _step_disc(self, obs_expert, obs_agent):
        ...
        run_tfs = [
            self._disc_grad_tf, self._disc_loss_tf, self._acc_expert_tf, self._acc_agent_tf, self._avg_prob_agent_tf,
