...
class NameServer:
    ...
    def __init__(self) -> None:
        ...
        self._gpu_table = {}
        self._uid_gpu_table = {}
        resource_config = read_config("config/hosts.yaml")
        ...
        self.logger = setup_logger("name_server")

        self._net = ZmqAdaptor(logger=self.logger)

        self.next_clean_up = time.time() + 180
    ...
    # 启动服务
    def run(self):
        config = read_config("config/name.yaml")
    ...
    def _discovery_train_server(self):
        self._discovery_rtype_server(rtype="train_server")
