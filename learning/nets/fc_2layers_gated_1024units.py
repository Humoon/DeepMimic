...
def build_net(input_tfs, reuse=False):
    ...
    bias_scale_kernel_init = tf.glorot_uniform_initializer()
    ...
    for i, size in enumerate(layers):
        ...
        with tf.variable_scope("gate{:d}".format(i), reuse=reuse):
            if (len(gate_layers) == 0):
                ...
                curr_gate_h_tf = TFUtil.fc_net(
                    input=gate_common_tf, layers_sizes=gate_layers, activation=activation, reuse=reuse)
                curr_gate_h_tf = activation(curr_gate_h_tf)

            curr_gate_bias_tf = tf.layers.dense(
                inputs=curr_gate_h_tf,
                units=size,
                kernel_initializer=bias_scale_kernel_init,
                bias_initializer=tf.zeros_initializer(),
                activation=None)

            curr_gate_scale_tf = tf.layers.dense(
                inputs=curr_gate_h_tf,
                units=size,
                kernel_initializer=bias_scale_kernel_init,
                bias_initializer=tf.zeros_initializer(),
